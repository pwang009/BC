...
namespace BlockChainCourse.Cryptography
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            const string nVersion = "0x20000000";
            byte[] bytes = BitConverter.GetBytes(Convert.ToInt32(nVersion, 16));
            //string retval = "";
            //foreach (byte b in bytes)
            //    retval += b.ToString("X2");
            Console.WriteLine($"Original String: {nVersion}");
            //Console.WriteLine($"Convert: {i}");
            ////Console.WriteLine($"UINT Parse: {UInt32.Parse(s)}");
            //Console.WriteLine($"Little endian: {retval}");
            Console.WriteLine($"BitConverter: {bytes.ByteArrayToHex()}");
            //Console.WriteLine($"Extension Method: {nVersion.ToLittleEndian('v')}");
        }
        ...
        private static void SHA256(string originalMessage, string originalMessage2)
        {
            ...
            var sha256HashedMessage = originalMessage.ComputeHashBySHA256();
            ...
            Console.WriteLine($"Message 1 hash {sha256HashedMessage}");
        ...
        private static void HMAC(string originalMessage, string originalMessage2)
        {
            ...
            var key = Hmac.GenerateKey(64);

            var hmacSha256Message = Hmac.ComputeHmacsha256(Encoding.UTF8.GetBytes(originalMessage), key);
            //var hmacSha256Message = Hmac.ComputeHmacsha256(Encoding.UTF8.GetBytes(originalMessage));

            var key2 = Hmac.GenerateKey(64);
            var hmacSha256Message2 = Hmac.ComputeHmacsha256(Encoding.UTF8.GetBytes(originalMessage2), key2);
            //var hmacSha256Message2 = Hmac.ComputeHmacsha256(Encoding.UTF8.GetBytes(originalMessage2));

            Console.WriteLine();
            Console.WriteLine("SHA 256 HMAC");
            Console.WriteLine();
            Console.WriteLine("Message 1 hash = " + Convert.ToBase64String(hmacSha256Message));
            Console.WriteLine($"HMAC Key is = {Convert.ToBase64String(key)}");
            Console.WriteLine("Message 2 hash = " + Convert.ToBase64String(hmacSha256Message2));
            Console.WriteLine($"HMAC Key2 is = {Convert.ToBase64String(key2)}");
